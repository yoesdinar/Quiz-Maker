...
export class AttemptRepository implements IAttemptRepository {
  ...
  async submitAttempt(id: number): Promise<Attempt> {
    return axiosClient.post<Attempt>(API_ENDPOINTS.ATTEMPT_SUBMIT(id));
  }

  async getAttemptAnswers(attemptId: number): Promise<AttemptAnswer[]> {
    return axiosClient.get<AttemptAnswer[]>(API_ENDPOINTS.ATTEMPT_ANSWERS(attemptId));
  }

  async submitAnswer(attemptId: number, answer: SubmitAnswerDto): Promise<void> {
    return axiosClient.post<void>(API_ENDPOINTS.ATTEMPT_ANSWERS(attemptId), answer);
  }
